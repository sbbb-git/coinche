...
export function CoachBar() {
  ...
  if (game.settings.trumpHelper && game.phase === "playing" && mode && mode !== "NT" && mode !== "AT") {
    ...
    trumpLine = t("coach.trumpsFallen", { seen });
  }
    ...
    <div className="px-2">
      <div className="mx-auto flex max-w-lg flex-wrap items-center justify-center gap-2">
        {myTurn && (
          ...
          >
            {hintLoading ? t("coach.hintLoading") : t("coach.hint")}
          </button>
        )}
        <button
      ...
      <div aria-live="polite" className="sr-only">
        {hintLoading ? t("coach.searching") : hint ? hint.text.replace(/\*\*/g, "") : ""}
      </div>
      {hint && (
...
function BidRecap({ game }: { game: GameState }) {
  ...
  return (
    <div id="bid-recap" className="mx-auto mt-2 max-w-lg rounded-xl bg-black/40 p-2.5 ring-1 ring-white/10">
      <p className="mb-1 text-xs font-semibold uppercase tracking-wide text-white/60">{t("coach.bidsTitle")}</p>
      {game.bidHistory.length === 0 ? (
        <p className="text-sm text-white/60">{t("coach.noBids")}</p>
      ) : (
          ...
          {game.bidHistory.map((b, i) => {
            ...
            return (
              <span key={i} className="tabular-nums">
                ...
                {b.kind === "pass" ? (
                  <span className="text-white/60">{t("coach.pass")}</span>
                ) : b.kind === "coinche" ? (
                  <b className="text-orange-300">{t("coach.coinche")}</b>
                ) : b.kind === "surcoinche" ? (
                  <b className="text-orange-300">{t("coach.surcoinche")}</b>
                ) : (
                  <b>
                    {b.generale ? t("coach.generale") : b.capot ? t("coach.capot") : b.value}
                    {m && (
                      <>
                        {" "}
                        {m.suit ? (
